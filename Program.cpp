...
void Program::setShaderNames(std::string const &v, std::string const &f,
                             std::string tc, std::string te) {
    vShaderName = v;
    fShaderName = f;
    teShaderName = te;
    tcShaderName = tc;
...
void Program::setProgramName(const std::string &name) { programName = name; }

std::string Program::getProgramName() { return programName; }
...
bool Program::init() {
    ...
    if (teShaderName.size() > 5) {
        teShaderString = readFileAsString(teShaderName);
        ...
        if (!rc) {
            if (isVerbose()) {
                ...
                std::cout << "Error compiling TE shader " << teShaderName
                          << std::endl;
            }
            return false;
        }
    ...
    if (tcShaderName.size() > 5) {
        tcShaderString = readFileAsString(tcShaderName);
        ...
        if (!rc) {
            if (isVerbose()) {
                ...
                std::cout << "Error compiling TC shader " << tcShaderName
                          << std::endl;
            }
            return false;
        }
    ...
    if (!rc) {
        if (isVerbose()) {
            ...
            std::cout << "Error compiling vertex shader " << vShaderName
                      << std::endl;
        }
        return false;
    }
