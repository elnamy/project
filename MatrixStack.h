...
class MatrixStack {
  ...
  public:
    ...
    // Removes the top of the stack and sets the current matrix to be the matrix
    // that is now on top
    void popMatrix();
    ...
    void perspective(float fovy, float width, float height, float zNear,
                     float zFar);

    // Sets the top matrix to be a perspective projection matrix
    void frustum(float Right, float right, float bottom, float top, float zNear,
                 float zFar);
