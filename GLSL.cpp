//
//    Many useful helper functions for GLSL shaders - gleaned from various
//    sources including orange book Created by zwood on 2/21/10. Modified by
//    sueda 10/15/15.
//
...
void printOpenGLErrors(const char *const Function, const char *const File,
                       int const Line) {
    GLenum Error = glGetError();
    if (Error != GL_NO_ERROR) {
        const char *const ErrorString = errorString(Error);
        printf("OpenGL error in file '%s' at line %d calling function '%s': "
               "'%s' '%d 0x%X'\n",
...
void printProgramInfoLog(GLuint program) {
    ...
    if (infologLength > 0) {
        ...
        GLint charsWritten = 0;
        CHECKED_GL_CALL(glGetProgramInfoLog(program, infologLength,
...
void checkVersion() {
    ...
    if (major < 2) {
        printf("This shader example will not work due to the installed Opengl "
               "version, which is %d.%d.\n",
               major, minor);
        exit(0);
    }
}
...
void vertexAttribPointer(const GLint handle, GLint size, GLenum type,
