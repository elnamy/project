...
MatrixStack::MatrixStack() { stack.push(glm::mat4(1.0)); }
...
void MatrixStack::perspective(float fovy, float width, float height,
                              float zNear, float zFar) {
    assert(zFar != zNear);
...
void MatrixStack::lookAt(const glm::vec3 &eye, const glm::vec3 &target,
                         const glm::vec3 &up) {
    glm::mat4 &top = stack.top();
    top *= glm::lookAt(eye, target, up);
}

const glm::mat4 &MatrixStack::topMatrix() const { return stack.top(); }
...
void MatrixStack::print(const char *name) const { print(stack.top(), name); }
