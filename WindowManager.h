...
class EventCallbacks {
  ...
  public:
    ...
    virtual void mouseCallback(GLFWwindow *window, int button, int action,
                               int mods) = 0;

    virtual void resizeCallback(GLFWwindow *window, int in_width,
                                int in_height) = 0;

    virtual void scrollCallback(GLFWwindow *window, double deltaX,
                                double deltaY) = 0;

    virtual void cursorCallback(GLFWwindow *window, double xpos,
                                double ypos) = 0;
};
...
class WindowManager {
  ...
  private:
    ...
    static void mouse_callback(GLFWwindow *window, int button, int action,
                               int mods);
    static void resize_callback(GLFWwindow *window, int in_width,
                                int in_height);
    static void scroll_callback(GLFWwindow *window, double in_deltaX,
                                double in_deltaY);
